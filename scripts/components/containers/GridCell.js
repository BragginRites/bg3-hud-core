...
export class GridCell extends BG3Component {
    ...
    get item() {
        ...
        // return (async () => BG3UTILS.getItem.bind(this)(this.data?.item));
        // return (async () => {
        //     if(!this.data.item) return;
        //     if(this.data.item.slug) return this.actor.system.actions?.find(a => a.slug === this.data.item.slug);
        //     else if(this.data.item.uuid) return await fromUuid(this.data.item.uuid);
    ...
    async getItemUses() {
        ...
        // if(this.data.item.override?.uses) return {uses: this.data.item.override.uses};
    ...
    async useItem(item, event, options) {
        return null;
